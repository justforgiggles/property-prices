...
export async function predictValuation(property: {
  ...
  const [encoders, [lowSession, highSession, recommendedSession]] = await (modelPromise ??= Promise.all([
    readFile(join(MODEL_DIRECTORY, "encoders.json"), "utf8").then((json) => JSON.parse(json) as Encoders),
    Promise.all([
  ...
  const features = {
    ...
    te_country: getEncodedValue(encoders.target_encoding, [["country", "South Africa"]], encoders.global_mean),
    te_locality_1: getEncodedValue(
      encoders.target_encoding,
      [["locality_1", property.locality_1], ["region", property.region]],
    ...
    te_region: getEncodedValue(encoders.target_encoding, [["region", property.region]], encoders.global_mean),
    te_type: getEncodedValue(encoders.target_encoding, [["type", property.type]], encoders.global_mean),
    total_rooms: property.bedrooms + property.bathrooms,
  };
  const values = encoders.feature_order.map((feature) => features[feature as keyof typeof features]);
  ...
  const [lowLog, highLog, recommendedLog] = await Promise.all(
    [lowSession, highSession, recommendedSession].map((session) =>
      session.run({ [session.inputNames[0]]: tensor }).then((outputs) =>
        Number(outputs[session.outputNames[0]].data[0]),
      ),
    ),
  );
...
export async function predict(request: Request, response: Response): Promise<void> {
  ...
  const property = {
    ...
    locality_2: (Reflect.get(request.body, "locality_2") as string | undefined)?.trim() ?? "",
